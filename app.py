...
def make_stream_token(item: QueuedVideo) -> str:
    now = int(time.time())
...
def make_stream_url(item: QueuedVideo, audio_track: Optional[int] = None, multi_audio: bool = False) -> str:
    token = quote(make_stream_token(item), safe='')
    url = f"{CPANEL_MEDIA_BASE}/media?token={token}"
    if multi_audio:
        url += "&multi_audio=1"
    if audio_track is not None:
        url += f"&multi_audio=1&audio_track={int(audio_track)}"
    return url
...
async def get_embedded_audio_tracks(item: QueuedVideo) -> list[dict[str, Any]]:
    if not CPANEL_MEDIA_BASE:
        return []
    token = make_stream_token(item)
    try:
        async with httpx.AsyncClient(timeout=45) as http:
            r = await http.get(f"{CPANEL_MEDIA_BASE}/tracks", params={"token": token})
            r.raise_for_status()
            tracks = r.json()
        result: list[dict[str, Any]] = []
        for track in tracks if isinstance(tracks, list) else []:
            if str(track.get("codec_type", "audio")).lower() != "audio":
                continue
            index = int(track.get("index", -1))
            if index < 0:
                continue
            label = str(track.get("label") or track.get("language") or f"Audio {len(result) + 1}").strip()
            result.append({"index": index, "label": label})
        return result
    except Exception as exc:
        logger.error("Embedded audio track discovery failed: %s", exc)
        return []
...
async def finish_mbulk(chat_id: int, session: dict[str, Any]) -> None:
    await send_bot_message(chat_id, "⏳ Multi-audio links are being created. Original video rahega; audio tracks player par on-demand load honge.")
    results: list[str] = []
    errors: list[str] = []
    for item in session["queue"]:
        season, episode, quality, _caption_audio = detect(item.caption)
        if episode is None:
            errors.append("Episode number not detected for one video.")
            continue
        if quality is None:
        ...
        if ok:
            ...
            errors.append(f"Catalog sync failed for {item.filename}.")
            logger.error("cPanel multi-audio ingest failed: %s", error)
    for slug in results:
        await send_bot_message(chat_id, "✅ Multi-audio episode ready\n\n" + slug + f"\n\n🔗 {CPANEL_WATCH_URL.split('/watch.php')[0]}/e/{quote(slug)}")
    for error in errors:
        await send_bot_message(chat_id, error)
    await send_bot_message(chat_id, f"✅ Multi-audio complete.\n\nTotal videos: {len(session['queue'])}\nReady episodes: {len(results)}")
...
async def process_update(update: dict[str, Any]) -> None:
    ...
    if command == "/mbulk" or command.startswith("/mbulk@"):
        ...
        if not prefix:
            ...
            return
        bulk_sessions[chat_id] = {"prefix": prefix, "queue": [], "multi_audio": True}
    ...
    if text.lower() == "/done":
        ...
        if not session:
            ...
            return
        if session.get("multi_audio"):
            await finish_mbulk(chat_id, session)
        else:
    ...
    if chat_id in bulk_sessions:
        ...
        if bulk_sessions[chat_id].get("multi_audio"):
            await send_bot_message(chat_id, f"✅ Multi-audio video added.\nDetected: S{season:02d} / Episode {episode} / {quality}\n\nSend more videos. When finished, send /done.")
        else:
